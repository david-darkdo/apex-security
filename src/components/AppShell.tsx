  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      {trustFeatures.length > 0 && (
        <section className="fixed bottom-12 md:bottom-0 left-0 right-0 z-20 border-t border-border bg-surface/95 py-2.5 shadow-md overflow-hidden backdrop-blur select-none">
          <style>{`
            @keyframes marquee {
            ...
            {trustFeatures.map((t) => {
              ...
              return (
                <div key={`${t.id}-1`} className="flex gap-2.5 items-center shrink-0">
                  <div className="rounded-full bg-brand-blue-soft p-1.5 text-brand-blue shrink-0">
...
function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-surface/95 backdrop-blur shadow-xs">
      <div className="container-app flex items-center gap-4 py-3">
        <Link to="/" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            <span className="font-display text-base sm:text-lg font-bold tracking-tight text-brand-orange leading-none uppercase">
              {companyName}
            </span>
        ...
        >
          ...
          <input
            ...
            className="w-full rounded-full border border-border bg-surface-elevated py-2 pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground outline-none transition focus:border-brand-orange focus:bg-surface focus:ring-1 focus:ring-brand-orange"
          />
        </form>
        ...
        <nav className="hidden md:flex items-center gap-1.5 shrink-0">
          <Link
            to="/home"
            className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-surface-elevated px-3 py-1.5 text-xs font-bold text-foreground hover:bg-brand-orange-soft hover:text-brand-orange hover:border-brand-orange transition shadow-2xs"
          >
        ...
        <div className="flex items-center gap-2.5">
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="relative grid h-9 w-9 place-items-center rounded-full border border-border bg-surface-elevated text-foreground transition hover:border-brand-orange"
              >
                <Bell className="h-4 w-4 text-brand-orange" />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-brand-orange text-[8px] font-bold text-canvas shadow-sm animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              {showNotifications && (
                <>
                  ...
                  <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-[340px] rounded-xl border border-border-strong bg-surface-elevated shadow-2xl p-4 text-xs space-y-3 z-50 md:absolute md:top-auto md:left-auto md:right-0 md:translate-x-0 md:translate-y-0 md:mt-2 md:w-80 md:rounded-lg md:shadow-xl md:p-4">
                    <div className="flex items-center justify-between border-b border-border pb-2">
                      <span className="font-bold text-foreground text-sm md:text-xs">Notifications</span>
                      <button onClick={() => setShowNotifications(false)} className="text-muted-foreground hover:text-foreground p-1 rounded-full hover:bg-surface"><X className="h-4 w-4" /></button>
                    </div>

                    <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                      {notifications.map((notif) => (
                        <div key={notif.id} className="p-2.5 border border-border rounded bg-surface flex gap-2 relative group text-left">
          ...
          <div className="relative">
            <button
              ...
              className="grid h-9 w-9 place-items-center rounded-full border border-border bg-surface-elevated text-brand-orange hover:border-brand-orange"
            >
              <User className="h-4 w-4" />
            </button>
            {menuOpen && (
              <div className="absolute right-0 mt-2 w-52 rounded-lg border border-border-strong bg-surface-elevated py-1.5 shadow-xl z-50">
                <Link
                  to="/collection"
                  search={{ autoPush: false }}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 px-4 py-2 text-sm text-foreground hover:bg-brand-orange-soft hover:text-brand-orange"
                >
                  <Bookmark className="h-4 w-4 text-brand-orange" />
                  <span>Active Workspace</span>
                </Link>
                <Link
                  to="/my-collections"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 px-4 py-2 text-sm text-foreground hover:bg-brand-blue-soft hover:text-brand-blue"
                >
        ...
        {nav.map((t) => (
          <Link
            ...
            className={`flex flex-col items-center gap-0.5 text-[10px] font-medium transition ${
              t.active ? "text-brand-orange font-bold" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            <div className="relative">
              <t.icon className="h-5 w-5" />
              {t.label === "Collection" && collectionCount > 0 && (
                <span className="absolute -top-1.5 -right-2 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-brand-orange text-[8px] font-bold text-canvas shadow-sm">
                  +{collectionCount}
                </span>
              )}
            </div>
